...
import com.wolfd.citygen.util.STL;
import com.wolfd.citygen.util.STLOutput;
...
//		}
		if(cs.length >= 3){
			...
			for(Geometry tri: gBase){
				//base
				stl += STL.normal(tri.getCoordinates()[0], tri.getCoordinates()[1], tri.getCoordinates()[2]);
				stl += "outer loop\n";
				stl += STL.vertex(tri.getCoordinates()[0]);
				stl += STL.vertex(tri.getCoordinates()[1]);
				stl += STL.vertex(tri.getCoordinates()[2]);
				stl += "endloop\n";
				stl += "endfacet\n";
